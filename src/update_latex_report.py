...
\begin{itemize}
    \item \textbf{2m Temperature (t2m):} The model achieves a strong linear correlation, with points in the scatter plot (Figure 11.2) clustering tightly along the diagonal. The raw model underpredicted extreme cold temperatures below $245$ K due to spatial smoothing, which was corrected by the post-processing bias correction layer.
\end{itemize}
...
The climatological drift analysis (Figure 11.5) over a 12-month period reveals that the model's predictions gradually smooth out extreme high-frequency variations over longer lead times. This is a common characteristic of models trained with mean squared error (MSE) loss, which optimizes for the mean state. While the model remains stable, this smoothing effect reduces the variance of forecasts at longer horizons.
...
The Bharati model was trained on CPU with single-threaded locks. The training loss converged steadily, starting at $0.6542$ and decreasing to $0.1865$ by Epoch 30 (Figure 11.6). The validation loss tracked the training loss closely, confirming the stability of the AFNO architecture on the larger spatial domain.

\section{Bharati Forecast Inferences}
\begin{itemize}
    \item \textbf{Temperature Scatter (Figure 11.7):} The model matches observations across the $235$ K to $280$ K range. The lower temperature range ($<240$ K) exhibits slightly higher variance, likely due to localized katabatic flows over the coastal slopes.
    \item \textbf{Wind Speed Scatter (Figure 11.8):} Wind speed predictions show larger errors during extreme events ($>18$ m/s) due to localized wind acceleration over the coastal topography.
    \item \textbf{Storm Meteorogram (Figure 11.9):} The model successfully tracked a severe coastal storm, capturing the rapid wind speed variations and corresponding temperature changes.
\end{itemize}
...
% ==========================================
% CHAPTER 11: METEOROLOGICAL VISUALIZATION AND PERFORMANCE CATALOG (NEW)
% ==========================================
