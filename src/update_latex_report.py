...
\begin{itemize}
    \item \textbf{2m Temperature (t2m):} The model achieves a strong linear correlation, with points in the scatter plot (Figure 12.2) clustering tightly along the diagonal. The raw model underpredicted extreme cold temperatures below $245$ K due to spatial smoothing, which was corrected by the post-processing bias correction layer.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Temperature Scatter (Figure 12.7):} The model matches observations across the $235$ K to $280$ K range. The lower temperature range ($<240$ K) exhibits slightly higher variance, likely due to localized katabatic flows over the coastal slopes.
    \item \textbf{Wind Speed Scatter (Figure 12.8):} Wind speed predictions show larger errors during extreme events ($>18$ m/s) due to localized wind acceleration over the coastal topography.
    \item \textbf{Storm Meteorogram (Figure 12.9):} The model successfully tracked a severe coastal storm, capturing the rapid wind speed variations and corresponding temperature changes.
\end{itemize}
...
% ==========================================
% CHAPTER 10: HIGH-PERFORMANCE COMPUTING (HPC) DEPLOYMENT (NEW)
% ==========================================
\chapter{High-Performance Computing (HPC) Deployment and Kernel Optimization}
...
\section{HPC Infrastructure and Scheduler Integration}
...
A sample production-grade Slurm submission script (\texttt{submit\_job.sh}) is detailed below:
\begin{lstlisting}[language=bash, caption=Slurm Job Submission Script for FourCastNet Training]
#!/bin/bash
#SBATCH --job-name=fourcastnet_polar
...
# Load system CUDA and compiler modules
module load cuda/12.1
module load cudnn/8.9
module load conda/latest

# Activate conda virtual environment
source activate fourcastnet_env

# Optimize CPU thread locks for data loaders
export OMP_NUM_THREADS=8
export MKL_NUM_THREADS=8

# Run model training with mixed precision
...
\begin{lstlisting}[language=bash, caption=Environment Variables for NCCL and DDP]
# NCCL and PyTorch distributed environment configurations
export MASTER_ADDR="10.0.0.1"
export MASTER_PORT=12345
export WORLD_SIZE=4
export RANK=0

# Force NCCL transport to use InfiniBand interfaces
export NCCL_DEBUG=INFO
export NCCL_IB_DISABLE=0
\end{lstlisting}

\section{Mixed Precision and Kernel Optimization}
...
\begin{enumerate}
    \item Forward pass weights and activations are cast to 16-bit floating-point (FP16 or BF16) representation, doubling the effective VRAM memory bandwidth.
    \item Loss scaling is applied to prevent numerical underflow in the gradient calculations:
    \begin{equation}
        \mathcal{L}_{\text{scaled}} = S \cdot \mathcal{L}
    \end{equation}
    where $S = 2^{16}$ is the initial dynamic scaling factor.
    \item Gradients are unscaled before the optimizer step to ensure update stability.
\end{enumerate}

Additionally, PyTorch's compiler kernel compilation (\texttt{torch.compile(model, mode="max-autotune")}) is utilized to fuse pointwise operations, Layer Normalizations, and activation functions, avoiding intermediate memory read/write cycles to the GPU Global Memory.

\section{Alternative Execution Kernels}
...
\begin{itemize}
    \item \textbf{Single-threaded CPU Lock:} For workstation dry-runs and inference tasks, setting \texttt{OMP\_NUM\_THREADS=1} and \texttt{MKL\_NUM\_THREADS=1} eliminates thread context-switching overhead and prevents CPU core overheating.
    \item \textbf{Singularity/Apptainer Containerization:} To ensure reproducibility across heterogeneous HPC environments with different host driver versions, the software stack is packaged into a Singularity container, executing as:
    \begin{lstlisting}[language=bash, caption=Singularity Execution Command]
singularity exec --nv fourcastnet.sif python src/inference.py --station bharati
    \end{lstlisting}
\end{itemize}

% ==========================================
% CHAPTER 11: LOCAL WORKSTATION SETUP AND WORKFLOW (NEW)
% ==========================================
\chapter{Local Workstation Setup and Workflow}
This chapter provides a step-by-step setup guide for deploying and running the FourCastNet regional forecasting model on a local workstation or laptop from scratch. It assumes no pre-installed dependencies other than Python.

\section{Repository Cloning and Environment Setup}
To begin, clone the repository and initialize a Python virtual environment to manage dependencies locally:
\begin{lstlisting}[language=bash, caption=Repository Setup and Environment Initialization]
...
\section{Retrieving the ERA5 Meteorological Dataset}
The model requires GRIB format ERA5 surface data. The Copernicus Climate Data Store (CDS) API is used for retrieval. 
\begin{enumerate}
    \item Register for a free account at \href{https://cds.climate.copernicus.eu}{Copernicus Climate Data Store}.
    \item Retrieve your API User ID (UID) and Key from your profile page.
    \item Create a configuration file named \texttt{\_cdsapirc} (or \texttt{.cdsapirc} on Linux) inside your user profile home directory (e.g., \texttt{C:\textbackslash{}Users\textbackslash{}Username\textbackslash{}} on Windows or \texttt{\textasciitilde{}/} on Linux) with the following format:
    \begin{lstlisting}[caption=.cdsapirc Configuration Format]
url: https://cds.climate.copernicus.eu/api/v2
key: YOUR_API_UID:YOUR_API_KEY
    \end{lstlisting}
    \item Execute the retrieval script in Python to download GRIB files locally:
    \begin{lstlisting}[language=Python, caption=Downloading ERA5 Surface Variables]
# Run python to execute retrieval (script code provided in Appendix B)
...
\begin{lstlisting}[language=bash, caption=Pre-processing the Datasets]
# Preprocess the regional station datasets (e.g. Maitri)
python src/preprocess_maitri_safe.py
\end{lstlisting}
This command converts raw GRIB files into the padded NumPy arrays required for model operations, saving the output files directly into the \texttt{data/} directory.

\section{Running Inference and Dashboard Generation}
With the pre-trained checkpoints in place, local forecasting, validation, and visual dashboards can be generated.
\begin{enumerate}
    \item **Run Forecast Inference:** Generate multi-step autoregressive weather predictions (out-of-sample evaluations):
    \begin{lstlisting}[language=bash, caption=Running Autoregressive Forecast Inference]
python src/inference.py --station maitri
    \end{lstlisting}
    Output files will be written to \texttt{checkpoints/maitri/} and validation metrics will be generated.
    \item **Generate Visual Dashboards:** Generate training history convergence, temperature scatter, wind speed scatter, severe storm meteorograms, and monthly climatology drift plots:
    \begin{lstlisting}[language=bash, caption=Dashboard and Graphs Generation]
python src/generate_dashboards.py
    \end{lstlisting}
    All high-resolution outputs will be saved directly to the respective station figures folder: \texttt{figures/maitri/dashboards/} and \texttt{figures/bharati/dashboards/}.
\end{enumerate}

% ==========================================
% CHAPTER 12: METEOROLOGICAL VISUALIZATION AND PERFORMANCE CATALOG (NEW)
% ==========================================
